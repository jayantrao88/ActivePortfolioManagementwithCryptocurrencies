---
title: "5d Backtesting Conventional CryptoFund"
output:
...
DateStamp the Portfolio Weights ( This happens only once the first time)

```{r}


for (OptimizationType in 1:6){
     ConvPortfolioWeights[[OptimizationType]] <- 
        as.xts(ConvPortfolioWeights[[OptimizationType]], order.by = as.Date(TempDate))
 }
rm(OptimizationType)
```
...
SP500Evolution <- xts(x = matrix(data = NA, nrow = nrow(AssetReturns), ncol = 1), order.by = index(AssetReturns))
...
Initializing the variables -- Conventional PortfolioReturns and  Conventional Evolution
```{r, echo=FALSE}

ConvPortfolioReturns <- list()

ConvPortfolioEvolution <- list()
```
...
for (OptimizationType in 1:6){
     ...
     a <- lag.xts(ConvPortfolioWeights[[OptimizationType]])
     a <- a[-1,]
  ...
  ConvPortfolioReturns[[OptimizationType]] <-   Temp
  ConvPortfolioReturns[[OptimizationType]] <-
    sapply(ConvPortfolioReturns[[OptimizationType]],unlist)
  ConvPortfolioReturns[[OptimizationType]] <-
    as.xts(ConvPortfolioReturns[[OptimizationType]], order.by = c)
colnames(ConvPortfolioReturns[[OptimizationType]]) <-
  names(ConvPortfolioWeights[OptimizationType])
rm(c)
}
ConvPortfolioReturns<- do.call(cbind,ConvPortfolioReturns)
z<- xts(x = t(rep(0,ncol(ConvPortfolioReturns))), order.by = as.Date("2014-09-10")) # Don't forget to change the first investment date from 2014-09-10
colnames(z) <- colnames(ConvPortfolioReturns)
z <- rbind.xts(z,ConvPortfolioReturns) 
ConvPortfolioEvolution <-  cumprod(z+1)
rm(z)
...
ConvEvolution <- merge.xts(SP500Evolution,IndividualEvolution,ConvPortfolioEvolution)
ConvReturns <- merge.xts(SP500PeriodicReturns,AssetReturns,ConvPortfolioReturns) 

CAGR <- ConvEvolution[nrow(ConvEvolution),]^(pfactor/(nrow(ConvEvolution)-1)) - 1
CAGR <- setNames(as.numeric(CAGR), colnames(CAGR))
MEAN <- colMeans(ConvReturns[-1,], na.rm = TRUE)*pfactor
STD <- colStdevs(ConvReturns[-1,],na.rm = TRUE)*sqrt(pfactor)

g <- cbind(MEAN,STD,CAGR)
ConvPerformanceMetrics <- t(g)

rm(ConvEvolution, ConvReturns, CAGR, MEAN, STD,g)
